...
///  ADDITIONAL MODIFICATIONS NEEDED HERE

/* convert a chunk of the fastq file into kmers */
bool reads_to_kmers(chunk &c, flush_object *obj, std::string gapped_kmer_pattern)
{

	//obj->console->info("Haloo");
	
	// REPLACE HARD CODED PATTERN WITH THE COMMAND LINE ARGUMENT PATTERN
	//std::string kmer_shape = "7-70-7-70-7-70-7";

	std::string kmer_shape = gapped_kmer_pattern;
...
//start_read:
		...
		//	goto next_read;
		//{
		if (read.length() < total_length) // start with the next read if length is smaller than K
			goto next_read;
		{
			...
			for (uint64_t i = 0; i < read.length()-total_length+1; i++){
				uint64_t gap_kmer;
				bool valid;
				//obj->console->info(i);
				//obj->console->info(read.length());
				...
				if(valid)
				{
					reverse_gap_kmer = Kmer::reverse_complement(gap_kmer, fixed_length);
 ...
 */
static bool fastq_to_uint64kmers_prod(flush_object* obj, std::string gapped_kmer_pattern)
{
	file_pointer* fp;
	...
	while (num_files) {
		while (ip_files.pop(fp)) {
			if (reader::fastq_read_parts(fp->mode, fp)) {
				...
				if (!reads_to_kmers(c, obj, gapped_kmer_pattern)) {
...
// PASS THE K-MER SHAPE THROUGH HERE
