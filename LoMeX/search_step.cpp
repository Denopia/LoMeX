	...
	*/
	...
	for (int thread = 0; thread < threads; thread += 1)
	{
		...
		*/
		...
		std::deque< std::deque<uint8_t> > last4longkmers;
		std::deque< std::deque<uint8_t> > last4longkmersrev;
		int my_buffer;
		//std::vector<std::string> kmer_occurrences_file_paths;
		//int last_amigos = total_length % 4;
		...
		while(fastq_reader.get_reads_left())
		{
			...
			while (!fastq_reader.is_empty())
			{
				...
				//update_last4(last4longkmers, long_kmer_as_bits, last_amigos);
				//update_last4(last4longkmersrev, long_kmer_as_bits_rev, last_amigos);
